...
public class RejectTagHook implements PreRepositoryHook<TagHookRequest>
{
	...
	private RepositoryHookResult detectIllegalTags(TagHookRequest hookRequest) {
		...
		List<String> illegalTags = hookRequest.getRefChanges().stream()
			.map(change -> change.getRef().getId()).filter(id -> id.startsWith("refs/tags/"+DEFAULT_TAG_PREFIX))
			.collect(Collectors.toList());
		if (illegalTags.size()>0) {
			result=RepositoryHookResult.rejected("IllegalTagPush", MessageFormat.format("The following tags are reserved and cannot be pushed by username {0}: {1}", username ,illegalTags.stream().collect(Collectors.joining(", "))));
		}
		return result;
	}
}
